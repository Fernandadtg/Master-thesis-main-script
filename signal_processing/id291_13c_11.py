...
# -*- coding: utf-8 -*-
"""
Created on Wed Aug  3 12:56:28 2022
...
path= '/home/u/pyNMR/nmr_bruker_400/marzo24/id291_13c_11'        #'/home/u/pyNMR/id280_13c_01' #'#/home/u/pyNMR/07_12/id273_13c_8
info_path = glob.glob(os.path.join(path + str('/202203*')))[0] #info_path
print(info_path)

file_title = path.split('/home/u/pyNMR/nmr_bruker_400/marzo24/', maxsplit=1)[1]
print("file_title")
print(file_title)
...
ordered = natsort.natsorted(filtered)[1:]# 1:33
...
#list_ft_sum2=list_ft[::1]
i1= 190#150#200#50#150#200#209#int(8000/size_freq) #190
i2= 240#235#220#250#250#250#222#int(5000/size_freq) #260
no_repeat_B =sorted([*set(list6)])
print(no_repeat_B)
for el_2 in data_o_lb:
   plt.plot(data_freq_p[0],el_2)
#plt.title(str("FID average"), loc='center')
plt.xlabel('frequency (Hz)')
plt.ylabel('NMR signal (arb. units)')
plt.savefig(results_dir + f" average FID {file_title}.pdf")
plt.show()
#plt.xlim([190,230])
#plt.xlim([-10000,0])
plt.show()
...
for elem in range(len(ft_from_fid)):
   plt.plot(ft_from_fid[elem])
...
fft200=[fft109[k]*np.exp(-0j*float(auto_list_values[k])/180.*np.pi) for k in range(len(auto_list_values))] #*np.exp(-1j*float(phimax[m]))
...
plt.savefig(results_dir + f" average FFT whole {file_title}.pdf")
plt.show()


for i in range(len(fft200)): #(len(fft101)): len(fft200)
  plt.plot(data_freq_p[0],np.real(fft200[i]))#[i1:i2]
#plt.title(str("FFT average"), loc='center')
# plt.xlim([i1,i2])
plt.xlim([-8000,0]) #205,225 #210,225
#plt.ylim([-0.3*10**6,0.81*10**6])
#plt.title(str("FFT average"), loc='center')
plt.xlabel('frequency (Hz)') #'sweep field values (mT/ms)'
plt.ylabel('NMR signal (arb. units)')
  #plt.grid()
plt.savefig(results_dir + f" average FFT {file_title}.pdf")
plt.show()

plt.figure(figsize=(20,10))
paso=no_repeat_B[3]-no_repeat_B[1]
plt.scatter(no_repeat_B,fft200_integral)
plt.plot(no_repeat_B,fft200_integral,'red')
plt.xticks(np.arange(48.5,53.5,0.22))
...
with open("integ_id291.txt", "w") as output:
