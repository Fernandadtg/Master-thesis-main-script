...
# -*- coding: utf-8 -*-
"""
Created on Wed Aug  3 11:07:16 2022
...
path='/home/u/pyNMR/id280_13c_01'
info_path = glob.glob(os.path.join(path + str('/202205*')))[0] #info_path
print(info_path)

file_title = path.split('/home/u/pyNMR/', maxsplit=1)[1]
print("file_title")
print(file_title)
...
i1= 205#150#200#209#int(8000/size_freq) #190
i2= 225#250#250#222#int(5000/size_freq) #260
...
for i in range(len(fft200)): #(len(fft101)):
  ...
  plt.plot(data_freq_p[0],np.real(ft_from_fid[i]))#[i1:i2]fft200
  plt.title(str(file_title)+str("NMR signal FFT "), loc='center')
...
plt.plot(no_repeat_B,fft200_integral,'g')
plt.xticks(no_repeat_B)
plt.title(str(file_title)+str("_average"), loc='center')
plt.xlabel('sweep slope (mT/s)') #'sweep field values (mT/ms)'
plt.ylabel('NMR signal (arb. units)')
#plt.grid()
plt.savefig(results_dir + f" average {file_title}.pdf")
plt.show()

with open("integ_id280_13c_01.txt", "w") as output:
